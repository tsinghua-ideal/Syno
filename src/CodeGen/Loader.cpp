...
Loader::Loader(const std::filesystem::path& path, const std::string& symbol, std::size_t countInputs, std::size_t countKernels):
...
void Loader::call(const std::size_t expectedCountBuffers, void *pipeline, const std::vector<at::Tensor *>& buffers) const {
    ...
    for (std::size_t i = 0; at::Tensor *buffer: buffers) {
        ...
        wrapped.emplace_back(Halide::PyTorch::wrap_cuda<float>(*buffer));
        ++i;
    }
