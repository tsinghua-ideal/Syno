...
bool NormalStage::possibleToFinalizeByExperimenting() const {
    ...
    const ShapeDistance distance = FinalizeOp::Distance(
        current, sampler.getDesiredShape(), graph,
        {
            ...
            .requiresOnlyOddNumelIncrease = options.maxExpansionRepeatMultiplier == 1 && options.requiresOddKernelSizeInUnfold && ContractionOp::NoMoreContractions(graph, options.maximumTensors),
