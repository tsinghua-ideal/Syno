...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        for name, pack in training.items():
            # 24 hours: 86400 secs
            if time.time() - training[name]['time'] > 86400 * 10:
        ...
        if selected_pack is not None:
            ...
            response_json = {'name': selected_name,
                             'py': selected_pack['py'], 'dot': selected_pack['dot']}
...
if __name__ == '__main__':
    # Arguments.
    parser = argparse.ArgumentParser(description='KAS selector server')
    ...
    dirs = [os.path.join(args.path, f) for f in os.listdir(
        ...
        for path in dirs:
            ...
            with open(f'{prefix}.py', 'r') as py_file:
                with open(f'{prefix}.dot', 'r') as dot_file:
                    collects[path.split(
                        '/')[-1]] = {'py': py_file.read(), 'dot': dot_file.read()}
        return collects
