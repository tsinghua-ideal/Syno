...
def test_remove():
    ...
    assert (
        mcts.do_rollout()[1][0][1]._node._node._name == "final1"
        if trials[0][1]._node._node._name == "final2"
        else "final2"
    )
...
def test_mcts():
    ...
    print(
        "Tree after first two iterations",
        [(v, v.N) for _, v in mcts._treenode_store.items()],
    )
...
def test_lrave():
    ...
    path = receipt
    print(f"Sampled {trials} for {path}")
    mcts.back_propagate(receipt, 0.0, trials[0][0])
    ...
    root_visible_grandchild = root_visible_grandchildren[0][1]
    assert (
        root_visible_grandchild._node._node._name == "s_2"
    ), root_visible_grandchild._node._node._name
