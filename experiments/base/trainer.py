            ...
            if args.local_rank == 0:
                logger.info(
                    ...
                    'Data: {data_time.val:.3f} ({data_time.avg:.3f})'.format(
                        ...
                        rate=image.size(0) * args.world_size /
                        batch_time_m.val,
                        rate_avg=image.size(
                ...
                if pruning_milestones and progress in pruning_milestones and \
                    ...
                    raise RuntimeError(
                        f'Pruned by milestone settings at progress {progress}%')
...
def train(args, model, train_loader, eval_loader, search_mode: bool = False, proxy_mode: bool = False):
    ...
    if args.native_amp:
        ...
        if args.local_rank == 0:
            logger.info(
                f'Training with native Apex AMP (loss scale: {args.apex_amp_loss_scale})')
    ...
    if args.local_rank == 0 and not search_mode:
        ...
        output_dir = get_outdir(
            args.output if args.output else './output/train', name)
        ...
        with open(os.path.join(output_dir, 'args.json'), 'w') as file:
            json.dump(vars(args), fp=file, sort_keys=True, indent=4,
                      separators=(',', ':'), ensure_ascii=False)

    # Pruning after epochs.
    overall_pruning_milestones = None
    if args.kas_epoch_pruning_milestone:
        with open(args.kas_epoch_pruning_milestone) as f:
            overall_pruning_milestones = json.load(f)
        if args.local_rank == 0:
            logger.info(
                f'Milestones (overall epochs) loaded: {overall_pruning_milestones}')
    ...
    for epoch in range(start_epoch, sched_epochs):
        ...
        if epoch == 0 and search_mode and not proxy_mode and args.kas_first_epoch_pruning_milestone:
            with open(args.kas_first_epoch_pruning_milestone) as f:
                in_epoch_pruning_milestones = json.load(f)
            if args.local_rank == 0:
                logger.info(
                    f'Milestones (first-epoch loss) loaded: {in_epoch_pruning_milestones}')
        ...
        # Evaluate.
        eval_metrics = validate(args, model, eval_loader,
                                eval_loss_func, amp_autocast, logger)
        ...
        if saver is not None:
            ...
            best_metric, best_epoch = saver.save_checkpoint(
                epoch, metric=eval_metrics[args.eval_metric])
