...
class Block(nn.Module):
    ...
    def forward(self, x):
        attn_value = self.drop_path(
            self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.block(self.norm1(x)))
        x = x + attn_value
        mlp_value = self.drop_path(
        ...
        if self.training:
            ...
            current_scale = (torch.abs(attn_value).mean() /
                             torch.abs(mlp_value).mean()).item()
            self.prev_items += x.size(0)
            self.kernel_scale = (prev_sum + current_scale *
                                 x.size(0)) / self.prev_items
        return x
...
@register_model
def kas_van_tiny(pretrained=False, pretrained_cfg=None, **kwargs):
