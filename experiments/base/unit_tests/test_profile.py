...
def get_model_args(
    model_name: str, batch_size=2, kas_replace_placeholder=None
) -> SimpleNamespace:
...
def test_fc_profile(batch_size=2):
    ...
    assert (
        flops_orig == flops_manual == flops_gt
    ), f"FLOPs mismatch: {flops_orig} vs {flops_manual} vs {flops_gt}"
    assert (
        params_orig == params_manual == params_gt
    ), f"Params mismatch: {params_orig} vs {params_manual} vs {params_gt}"
