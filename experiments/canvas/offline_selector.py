...
if __name__ == '__main__':
    ...
    kernel_paths = []
    if args.rank == 0:
        print(f'Reading files in path {args.kas_selector_dir} ...')
        for name in os.listdir(args.kas_selector_dir):
            kernel_path = os.path.join(args.kas_selector_dir, name)
            if os.path.isdir(kernel_path):
                ...
                with open(os.path.join(kernel_path, f'{k}_{h}.json')) as f:
                    j = json.load(f)
                    if j['extra']['m_params'] <= args.kas_selector_max_params:
                        kernel_paths.append(kernel_path)
        kernel_paths = sorted(kernel_paths, reverse=True)
        print(f'{len(kernel_paths)} valid kernels (param threshold: {args.kas_selector_max_params}) collected.')
    else:
        kernel_paths = []
    ...
    for kernel_path in kernel_paths:
        ...
        if args.local_rank == 0 and args.kas_selector_save_dir:
            ...
            new_path = os.path.join(
                args.kas_selector_save_dir, f'KAS_{score_str}_Kernel_{h}')
