...
def build(bb: BlockBuilder, in_0: relax.Var, in_1: relax.Var, N: int = 1, C_in: int = -1, C_out: int = -1, H: int = -1, W: int = -1, k: int = 3, s: int = 2) -> relax.Var:
    ...
    def subgraph_0(in_0: te.Tensor) -> te.Tensor:
        ...
        return te.compute(
            ...
            lambda i_0, i_1, i_2, i_3:
                te.sum(in_0[i_0, i_1, i_2 * s + ri_0, i_3 * s + ri_1], axis=[ri_0, ri_1]),
            name="subgraph_0",
        )
    def subgraph_1(in_0: te.Tensor, in_1: te.Tensor) -> te.Tensor:
        ...
        return te.compute(
            ...
            lambda i_0, i_1, i_2, i_3:
                te.if_then_else(
                    i_2 + ri_0 - k // 2 >= 0 and i_2 + ri_0 - k // 2 < k and i_3 + ri_1 - k // 2 >= 0 and i_3 + ri_1 - k // 2 < k,
                    te.sum(in_0[i_0, ri_2, i_2 + ri_0 - k // 2, i_3 + ri_1 - k // 2] * in_1[i_1, ri_2, ri_0, ri_1], axis=[ri_0, ri_1, ri_2]),
                    0.0,
                ),
            name="subgraph_1",
        )
