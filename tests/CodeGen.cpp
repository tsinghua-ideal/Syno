...
protected:
  Sampler sampler = Sampler("[N,H,W]", "[N,H,W]", {"N=8", "H=16", "W=16"},
                            {"k=3", "s=2"}, {{}}, {{0, 0}});
...
TEST_F(codegen_tests, generate) {
  ...
  gen.generate("./kernel_codegen_test/forward",
               "./kernel_codegen_test/backward", "codegen_test_forward",
               "codegen_test_backward", consts, &std::cout);
}
