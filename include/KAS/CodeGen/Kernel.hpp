...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(KernelMetadata, name, inputsShapes, outputShape, vram, halide, cuda, countInputs, validPlaceholdersIndices, validPlaceholders);
...
public:
    ...
    Kernel(const BindingContext& ctx, const TensorView& tensorView, const PyTorchSpecializedIR& pyTorchSpecializedIR, const std::vector<std::map<std::string, std::size_t>>& allMappings, CodeGenOptions options, const std::filesystem::path& dir, const std::string& name);
